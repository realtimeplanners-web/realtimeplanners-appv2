// Demo authentication bypass - no Firebase/Google auth required
// Check for demo user and auto-populate user info
function setupDemoUser() {
  const demoUser = JSON.parse(localStorage.getItem('demoUser') || '{}');
  
  if (demoUser.id) {
    // Populate user info with demo data
    const name = demoUser.displayName || demoUser.email?.split('@')[0] || 'Demo User';
    
    document.getElementById('userName').textContent = name;
    document.getElementById('topbarUserName').textContent = name;
    
    const lastLogin = demoUser.lastSignInTime;
    if (lastLogin) {
...
// Initialize demo user when page loads
...
function doPaint(e, canvas) {
  ...
  if (isEraser) {
    ...
    const g = ctx.createRadialGradient(x,y,0,x,y,r);
    g.addColorStop(0, `rgba(${r2},${g2},${b2},0.6)`);
    g.addColorStop(0.5, `rgba(${r2},${g2},${b2},0.4)`);
    g.addColorStop(0.8, `rgba(${r2},${g2},${b2},0.2)`);
